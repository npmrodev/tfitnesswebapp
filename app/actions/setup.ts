...
export async function setupTestAccounts() {
  try {
    const accounts = [
      {
        email: process.env.OWNER_EMAIL || 'admin@tfitness.com',
        password: process.env.OWNER_PASSWORD || 'Admin@123',
        name: 'Admin User',
        role: 'owner',
        firstName: 'Admin',
        lastName: 'User',
      ...
      {
        email: process.env.STAFF_EMAIL || 'staff@tfitness.com',
      ...
      {
        email: process.env.MEMBER_EMAIL || 'member@tfitness.com',
        password: process.env.MEMBER_PASSWORD || 'Member@123',
        name: 'Member User',
        role: 'member',
        firstName: 'Member',
        lastName: 'User',
    ...
    return { success: true, message: 'Test accounts and gym settings initialized successfully' }
  } catch (error) {
